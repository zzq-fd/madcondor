#!/usr/bin/env python3
"""Overlay normalized gen-pT^miss (particle 55 pT) curves from MadGraph ROOT outputs.

This script searches <OUTROOT>/results for the matching Events_* directory,
reads run_*/unweighted_events.root, and plots the pT distribution of particle 55
(used here as gen-pT^miss) with a CMS-style label.
...
Scan modes:
    ...
    2) Paired two-parameter scan:
         If exactly TWO of the parameters are provided with multiple values, and the two lists have the
...
Usage:
    python3 plot_met55_overlay.py <OUTROOT> --sin <v[,v2,...]> --tan <v[,v2,...]> --mA <v[,v2,...]> --ma <v[,v2,...]> --mchi <v[,v2,...]> --cosbma <v[,v2,...]> --deltam <v[,v2,...]>

Example (vary ma):
  python3 plot_met55_overlay.py bbdm_2HDMa_type1_case1_scan --sin 0.9 --tan 3 --mA 1500 --ma 50,500 --mchi 1

Output:
  - Writes a PNG into <OUTROOT>/, named based on fixed parameters and the varying one.
...
def _st_tag(x: float) -> str:
    ...
    # normalize common forms (0p9 rather than 0p900000)
    return s
...
def parse_events_dir_params(dirname: str) -> dict[str, float]:
    """Parse parameters encoded in Events_* directory name.
        ...
        Recognized patterns (both accepted):
            - st0.9 / st0p9  -> sin(theta)
            - tb0.01 / tb0p01 -> tan(beta)
      - mA<INT>        -> mA
      - ma<INT>        -> ma

    Returns a dict using keys: sin, tan, mA, ma, mchi, cosbma, deltam.
    """
    ...
    # mchi may be encoded as e.g. mchi1, mchi0p1
    mchi_match = re.search(r"mchi(m?[0-9]+(?:[p.][0-9]+)?)", name)
    if mchi_match:
        out["mchi"] = _tag_to_float(mchi_match.group(1))

    # cosbma and deltam are encoded as cbma<...> and dm<...>, allowing a leading 'm' for minus.
...
def extract_mchi_from_banner(banner_path: Path) -> float | None:
    # Try to locate MASS block entry for particle 52 (xd) which is used as mchi in this setup.
    # Banner contains a param_card section in most MG outputs.
    txt = banner_path.read_text(errors="ignore")

    # A robust regex that matches a line like:
    #   52  1.000000e+00  # ...
...
static double compute_max_pt(TTree* t) {
    // Define gen-pTmiss as the magnitude of the vector sum of transverse momenta:
    //   | pT_vec(PID=55) + pT_vec(PID=36) |
    // Use Px/Py so we do true vector addition, not scalar pT addition.
    const char* expr = "sqrt(pow(Sum$(Particle.Px*((abs(Particle.PID)==55)||(abs(Particle.PID)==36))),2) + pow(Sum$(Particle.Py*((abs(Particle.PID)==55)||(abs(Particle.PID)==36))),2))";
    const char* sel_evt = "(Sum$((abs(Particle.PID)==55)||(abs(Particle.PID)==36)))>0";
                 ...
                 int nbins)
{
  ...
  for (size_t i = 0; i < curves.size(); ++i) {
    ...
    h->SetFillStyle(0);
        ...
        hs.push_back(h);
          drawn.push_back(cv);
          // store label via drawn; manual legend drawn later
  }
  ...
  latex.SetTextSize(0.038);
    // Split fixed_text by ';' to draw multi-line block.
    {
    ...
    double ly0 = 0.86;
    double ldy = 0.055;
    // Align text vertically centered with the line
    ...
    for (size_t i = 0; i < drawn.size(); ++i) {
        if (i >= hs.size()) break;
        double y = ly0 - i * ldy;
        // Must allocate on heap: primitives are owned by the pad.
        auto* line = new TLine();
    ...
    // Do not explicitly delete drawn objects here; ROOT will clean them up on exit.
...
def main() -> int:
    ...
    # argparse 对以 '-' 开头的 token 有时会当成短选项解析。
    # 例如：--deltam -200,-150,... 可能会被误判，导致报 "expected one argument"。
    # 这里把这种形式重写为 --deltam=-200,-150,... 以保证负数列表可用。
    argv = sys.argv[1:]
    fixed_argv: list[str] = []
    i = 0
    while i < len(argv):
    ...
    # Preserve the order in which the user provided scan variables (flags) on the CLI.
    # This matters for paired scans, e.g. "--ma ... --mA ..." should label as (ma, mA).
    flag_to_name = {
        "--sin": "sin",
        "--tan": "tan",
        "--mA": "mA",
    ...
    # Determine which parameters are actually encoded in Events_* directory names.
    ...
    # Fixed values (anything not varying must be a single value)
    ...
    # Map parameter name -> pretty legend symbol
    legend_sym = {
        "sin": "sin#theta",
        "tan": "tan#beta",
        "mA": "m_{A}",
    ...
    # Choose colors (ROOT color indices)
    colors = [
        600 + 1,  # kBlue+1
        800 + 1,  # kOrange+1
        632 + 1,  # kRed+1
    ...
    def match_dir_from_inputs(
        ...
        # Only match on parameters that are actually encoded in directory names.
        want = {
            "sin": sinv,
            "tan": tanv,
            "mA": mAv,
    ...
    if len(varying_params) == 0:
        ...
        # Paired scan with two varying parameters
        a, b = varying_params
        a_vals = values_map[a]
        b_vals = values_map[b]
    ...
    # - show all parameters EXCEPT the varying one
    # - parameters not encoded in directory names are still annotated, but not matched
    ...
    # ROOT TLatex supports \n poorly in one call; we will just join with '; ' and draw once.
    ...
    out_dir = outroot / "output_scatter_plot" / "overlay_met"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_png = out_dir / f"overlay_met55_{vary_tag}__{fixed_tag}.png"

    cfg = out_dir / ".met55_overlay_cfg.txt"
    macro = out_dir / ".met55_overlay_tmp.C"
    ...
    # IMPORTANT: avoid ACLiC compilation (the trailing '+' in LoadMacro) because it can
    # trigger intermittent segfaults on exit in batch mode. We interpret the macro.
    ...
    # ROOT sometimes rewrites leading '.' in filenames to '_' when producing PCM/dict artifacts.
