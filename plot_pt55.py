#!/usr/bin/env python3
"""Plot pT of particle 55 from MadGraph ROOT outputs and summarize mean pT.

Usage:
  python3 plot_pt55.py <OUTROOT>

Example:
  python3 plot_pt55.py bbdm_2HDMa_type1_case1_scan

What it does:
  - Scans <OUTROOT>/results/Events_*/*/unweighted_events.root
  - For each Events_* directory, makes a pT histogram for particle 55 and saves
    a PNG into that Events_* directory.
  - Computes mean pT for particle 55 and writes a summary table into <OUTROOT>/.
...
int plot_pt55(const char* input_root,
              ...
              double xmax_in)
{
  ...
  if (!t) {
    ...
    // Compute gen-pTmiss per event as the magnitude of the vector sum:
    //   | pT_vec(PID=55) + pT_vec(PID=36) |
    // Use Px/Py so we do true vector addition.
    ...
    const char* sel  = "(Sum$((abs(Particle.PID)==55)||(abs(Particle.PID)==36)))>0";
  ...
  if (n <= 0) {
    ...
    // Still write an empty plot (useful for bookkeeping)
        ...
        c.SetBottomMargin(0.12);
                ...
                TH1D h("h", ";gen-p_{T}^{miss} [GeV];Entries", 10, xlow, xhi);
    h.Draw();
    c.SaveAs(output_png);
    return 0;
  }
  ...
  for (Long64_t i = 0; i < n; ++i) {
    ...
    if (!(xhi > xlow)) xhi = xlow + 1.0;
        ...
        TH1D h("h", ";gen-p_{T}^{miss} [GeV];Entries", 100, xlow, xhi);
    double sum_sel = 0.0;
    Long64_t n_sel = 0;
  for (Long64_t i = 0; i < n; ++i) {
        const double pt = v[i];
...
def root_quote(path: Path) -> str:
    # ROOT interprets C++ string literals; escape backslash and quotes.
...
def find_root_file(events_dir: Path) -> Path | None:
    # Prefer run_01 if present; otherwise take the first run_* in lexical order.
...
def main() -> int:
    ...
    macro_path = outroot / ".plot_pt55_tmp.C"
    ...
    for ev_dir in event_dirs:
        ...
        out_png = ev_dir / "pt_particle55.png"
        out_txt = ev_dir / "pt_particle55_stats.txt"

        stmt = (
            f'gROOT->LoadMacro("{root_quote(macro_path)}+"); '
            f'plot_pt55("{root_quote(root_file)}","{root_quote(out_png)}","{root_quote(out_txt)}",{args.xmin},{args.xmax});'
        )
        cmd = [root_exe, "-l", "-b", "-q", "-e", stmt]

        # Capture output for debugging without spamming the terminal.
        log_path = ev_dir / "pt_particle55_root.log"
            ...
            for line in txt:
                ...
                if line.startswith("mean_pt "):
                    try:
                        ...
                        mean_pt = None if (val != val) else val  # NaN check
                    except Exception:
                        mean_pt = None
        ...
        if proc.returncode != 0:
            ...
            # Clean up per-point log on success to avoid clutter.
            # Keep it when there is a failure so you can debug.
            try:
    ...
    # Write summary table
    table_path = outroot / "pt55_mean_table.txt"
    with open(table_path, "w") as f:
        f.write("# Mean pT table for particle 55\n")
    ...
    # Clean up temporary ROOT macro and compilation artifacts on success.
    # ROOT creates files like:
    #   .plot_pt55_tmp_C.so, .plot_pt55_tmp_C.d, (sometimes) .plot_pt55_tmp_C.pcm
    # and sometimes ACLiC dictionary cache files like:
    #   _plot_pt55_tmp_C_ACLiC_dict_rdict.pcm
    if not any_failures:
        try:
        ...
        # Delete any files produced by ACLiC for this macro.
